...
**解析大纲：** 从根文件出发逐行解析。支持 `= 标题` 和 `#heading(level: N)[标题]` 两种写法。`#include "file.typ"` 和 `#import "file.typ"` 会递归进入子文件，将其标题插入文档顺序中。`@package` 包引用自动排除。注释行跳过，循环引用防护，无后缀 include 自动补全。无标题的文件也会显示入口。
...
如果您对这个插件有其他需求，或者认为这个插件有需要改进的地方，欢迎您提交 issue（不过大概率你提交了没啥用，我不一定有能力修改）。
